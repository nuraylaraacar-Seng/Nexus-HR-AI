...
function App() {
  ...
  // Sayfa açılınca tüm verileri paralel çek
  useEffect(() => {
  ...
  // DÜZELTME: totalEmployees artık API'den geliyor, magic number yok
  const totalEmployees = kpiData?.total_employees ?? 0;
  const riskRate = totalEmployees > 0
    ? `%${((riskList.length / totalEmployees) * 100).toFixed(0)}`
    : '...';
    ...
    <div style={styles.page}>
      ...
      <div style={styles.cardRow}>
        ...
        <div style={styles.card}>
          ...
          <p style={styles.cardValue}>{riskRate}</p>
        </div>
      </div>

      <div style={styles.twoCol}>
        {/* CİNSİYET MAAŞ UÇURUMU */}
